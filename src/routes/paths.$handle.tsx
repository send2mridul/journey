...
import { getOurPaths, getOurPathsShareData, proposeSharedMoment, setPairPermission } from '@/server/social';
...
function OurPathsRoute() {
  ...
  if(error)return <main className="paths-error"><LockKeyhole/><h1>Our Paths remains private.</h1><p>{error}</p><Link to="/circle"><ArrowLeft/>Return to Life Circle</Link></main>;
  if(!data)return <div className="social-loading">Finding where two stories overlap…</div>;
  return <><OurPathsView data={data} onPermission={(kind,allowed)=>void permission(kind,allowed)} onConfirmMoment={(discovery)=>void confirmMoment(discovery)} onShare={()=>void sharePaths()}/><OurPathsShareDialog open={shareOpen} onOpenChange={setShareOpen} preview={sharePreview}/>{message&&<div className="circle-toast" role="status">{message}</div>}</>;
}
